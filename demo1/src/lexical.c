...
struct lng_token_t* cgl_lexical(const unsigned char* text, unsigned int* tokenSize) {
	...
	while (c) {
		if (c <= ' ') {
			...
			switch (word[0]) {
                ...
                case 'd':
                    switch (size) {
                        case 2:
                            if (word[1] =='o') {
                                type = CGL_LEXICAL_DO_WORD;
                            }
                            break;
                        case 6:
                            if (word[1] =='o' && word[2] == 'u' && word[3] == 'b' && word[4] == 'l' && word[5] == 'e') {
                                type = CGL_LEXICAL_DOUBLE_WORD;
                            }
                            break;
                    }
                    break;
                    ...
                    switch (size) {
                        case 6:
                            if (word[1] =='e' && word[2] == 'p' && word[3] == 'e' && word[4] == 'a' && word[5] == 't') {
                                type = CGL_LEXICAL_REPEAT_WORD;
                            } else if (word[1] =='e' && word[2] == 't' && word[3] == 'u' && word[4] == 'r' && word[5] == 'n') {
                                type = CGL_LEXICAL_RETURN_WORD;
                            }
                            break;
                    }
                    break;
			...
			unsigned short type = CGL_LEXICAL_HEX;
			long begin = pc - text;
			pc++;
			c = *(++pc);
			while ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')) {
			...
			unsigned short type = CGL_LEXICAL_NUMBER;
			long begin = pc - text;
			c = *(++pc);
			col++;
			while (c >= '0' && c <= '9') {
			...
			int clin = lin;
			int ccol = col;
			unsigned short type = CGL_LEXICAL_STRING;
			long begin = pc - text;
			c = *(++pc);
			col++;
