...
char* cgl_io_uint_to_string(unsigned int value, char *digits, int base) {
	...
	if (value < (unsigned int) base) {
		...
		for (p = cgl_io_uint_to_string(value / ((unsigned int) base), digits, base); *p; p++) {
		}
		cgl_io_uint_to_string(value % ((unsigned int) base), p, base);
	}
	return digits;
}
...
char* cgl_io_int_to_string(int value, char *digits, int base) {
	...
	if (value < 0) {
		*d++ = '-';
		...
		u = value;
	cgl_io_uint_to_string(u, d, base);
	return digits;
}

void cgl_io_double_to_integer(double x, int* exp, int64* frac) {
